...
0.5.1 提供真正的全局猫爪跟随、悬停抬爪、按下/回弹、拖动和一次轻微待机晃动。暖白猫毛，粉色/咖啡色肉垫；箭头尖端和点击精度不变。

## 安装和控制

下载并解压 `CatPaw-0.5.1-Linux.tar.gz`，在解压目录运行：
...
悬停状态由 XFixes 提供的光标名称识别；没有设置标准名称的应用仍可跟随、点击，但可能无法识别 Link。文本状态使用更小、更靠近的猫爪，并禁用待机晃动；精确定位、缩放、Busy 等状态隐藏伴随猫爪，保留系统主题。Link 是光标角色，Click 是鼠标事件。

为避免双爪，运行时切换到 Companion 专用主题；正常停止后恢复原 CatPaw 主题。强制杀死/断电无法执行恢复，可再次启动后停止或重跑安装器。

XInput2 鼠标事件驱动，不轮询位置；动画约60fps，动作结束后取消刷新定时器；待机只执行一次轻晃。隔离 Xvfb 测试覆盖跟随、按下/回弹、拖动、悬停、文本紧凑布局、穿透输入区域，并确认静止时不增加动画帧及采样区间内 CPU tick。真实桌面的混合 DPI、全屏应用和合成效果仍需体验验证。

测试：`python3 companion/test_runtime.py`（需要 Xvfb 和 libXtst）；可通过 CATPAW_XVFB 指定虚拟服务器路径。测试只对自己的虚拟显示发送输入。

0.5.1：修正双轴重复叠加间距；加入追随侧倾、拍下压扁和松开轻跳；待机只轻抬并摇摆一次。输入状态缩小至65%，靠近 I-Beam 右下方，保留文字选择热点。此处跟随的是鼠标 I-Beam，不是文字插入位置。

框选卡顿修复：协商 XInput 2.1，保证其他应用抓取鼠标期间仍收到原始鼠标事件；选择文字时跟随延迟上限40ms，倾斜降至2°。回归测试先在旧协议下复现事件丢失，再验证修复后的抓取期间移动和释放。
