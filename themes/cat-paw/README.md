...
**Phase 1 · 视觉审核稿 v0.2.0。** 这次交付为可编辑 SVG、透明 PNG、预览与构建工具。尚未提供 Xcursor、CUR/ANI 安装包，也没有 Companion 程序。
...
猫爪采用略倾斜、非对称的姿态，通过暖白渐变、短毛束、底部暖阴影与肉垫高光表达柔软。24/32px 版本加粗轮廓并去掉细毛线与肉垫高光；24px 铃铛简化，32px 起保留蝴蝶结铃铛，48px 起恢复基础毛发层次，64px 起增加细毛线，而不是直接缩小一张 128px 图。

箭头重绘为更接近参考稿的圆润三角轮廓，保持左上尖端热点。末端蝴蝶结与金色小铃铛独立于猫爪。I-Beam 改为有实体厚度的暖白轮廓，顶部猫耳内侧加入肉垫配色。所有渐变均为 SVG 原生定义，不嵌入位图。
