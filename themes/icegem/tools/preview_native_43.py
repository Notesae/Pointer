"""独立的 4.3 造型/本体动画原型；只生成预览，不更改 CUR、ANI 或安装包。"""
...
def document(role, frame):
    ...
    gradients = [('table', '#c7ecff', '#71a8d7'), ('top', '#f2fcff', '#73b4e8'),
                 ('right', '#37669c', '#a3c4f2'), ('bottom', '#d0c9f5', '#5084be'),
                 ('left', '#4581b6', '#c1f0ff')]
    ...
    for i, name in enumerate(['top', 'right', 'bottom', 'left']):
        ...
        parts.append(f'<polygon points="{points([mid,INNER[(i+1)%4],INNER[i]])}" fill="#d8efff" opacity=".18"/>')
    ...
    parts.append(f'<polygon points="{points(OUTER)}" fill="none" stroke="#34598a" stroke-width=".65" stroke-linejoin="miter"/>')
    parts.append('<path d="M 4.5 3.8 L 21.6 11.4 M 10.5 20.8 L 27.3 28" fill="none" stroke="#dff6ff" stroke-width=".42"/>')
...
def build():
    ...
    for dark in (False, True):
        ...
        for frame in range(288):
            ...
            draw.text((20, 12), 'IceGem 4.3 · 本体动画原型 / 轮廓与热点固定', font=font, fill=fg)
            for role in range(4):
                x = role * 240
            ...
            frames.append(sheet)
        name = 'native-4.3-prototype' + ('-dark' if dark else '')
