...
try {
 ...
 if($hash -in $known){Write-Host '[MATCH] Active registry points to the 4.5 RosePink ROTATING CRYSTAL.' -ForegroundColor Green;Write-Host 'If only one app still shows the old icon, fully restart that app and test again.'}
