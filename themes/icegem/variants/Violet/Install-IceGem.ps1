...
Write-Host "IceGem 4.0 Violet installer | Source: $SourceRoot"
...
try {
    ...
    if($Action -in @('Restore','Uninstall')){
        ...
        if((Get-FileHash -LiteralPath $linkSource -Algorithm SHA256).Hash -ne $linkExpected[$Size]){
            throw 'Link resource is not the 4.0 Violet strong gem click. Extract the complete versioned package into a NEW folder.'
        }
        $before=Capture-Settings
        ...
        Write-Host '[VERIFIED] Link Select = 4.0 Violet STRONG GEM CLICK (file + registry).' -ForegroundColor Cyan
