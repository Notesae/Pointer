...
Write-Host "IceGem 4.5 IceBlue installer | Source: $SourceRoot"
...
try {
    ...
    if($Action -in @('Restore','Uninstall')){
        ...
        $linkExpected=@{
            '32'='EF0490166E5966AB139EE66D6CBE45CDDFEB9107EC9C49545547D1BD0101BCC5'
            '48'='B121D9677F6FCFEE64243CF7229E6DC70227EFE374DAF5CE4D5B5F9B98B58D0B'
            '64'='CBD16DAFF0C4151F5C27ACD74C00CDF9893AA0B51D4C14C3C64BD190659A286A'
            'multi'='44F7A9F1E28E465DC870F33E22AA7489A43535FEDADE23BEB89EC6C62B6EFA98'
        }
        ...
        if((Get-FileHash -LiteralPath $linkSource -Algorithm SHA256).Hash -ne $linkExpected[$Size]){
            throw 'Link resource is not the 4.5 IceBlue rotating crystal. Extract the complete versioned package into a NEW folder.'
        }
        $before=Capture-Settings
        ...
        Write-Host '[VERIFIED] Link Select = 4.5 IceBlue ROTATING CRYSTAL (file + registry).' -ForegroundColor Cyan
