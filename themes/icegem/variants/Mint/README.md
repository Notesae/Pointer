...
仓库源码目录需要先执行 `python tools/build.py` 生成 cursors；也可直接下载主题 dist 中的完整安装包。生成后双击本目录 Install.cmd 安装并应用 薄荷绿 动态方案。Windows 方案名为 IceGem Mint Gentle (multi)；Static 为静态版。各颜色方案独立命名，安装过的配色可在鼠标属性中切换。

18 种状态，32/48/64px 与 multi，14 种状态提供独立时序 ANI；正常为 96 个动作帧加停留帧，后台为 96 帧，等待为 36 帧。文本与竖排文本使用 75 帧 / 3 秒的水晶展台：紧凑切面底座、固定支撑柱、上端晶体自转浮动与倾摆，热点固定在柱身中心。Static 使用首帧；禁用符号保持红色，其他材质按主题配色。
