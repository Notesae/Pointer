...
本版与 Windows 4.4 使用相同造型和固定帧动效：不同状态采用不同旋转幅度，忙碌状态为三颗横向晶体依次放大并亮起。没有把 CUR/ANI 改扩展名，而是重新编码为 Linux Xcursor 文件。

[下载 Linux 4.4 完整安装包](../dist/IceGem-4.4-Linux.tar.gz?raw=true) · [SHA-256 校验值](../dist/IceGem-4.4-Linux.tar.gz.sha256)

## 安装：不需要 sudo

解压 `IceGem-4.4-Linux.tar.gz`，进入解压出的 `IceGem-Linux` 文件夹，在空白处右键“在终端打开”，执行：
